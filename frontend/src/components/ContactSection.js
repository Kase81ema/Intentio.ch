    ...
    <motion.form initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} onSubmit={handleSubmit} data-testid="contact-form">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          ...
          <Input id="nome" data-testid="form-nome" value={form.nome} onChange={(e) => set("nome", e.target.value)} className="mt-2 h-12 rounded-none border-slate-300 focus:border-violet-600" required />
        </div>
        <div>
          <Label htmlFor="azienda" className="text-slate-600 text-xs tracking-wide uppercase">Azienda *</Label>
          <Input id="azienda" data-testid="form-azienda" value={form.azienda} onChange={(e) => set("azienda", e.target.value)} className="mt-2 h-12 rounded-none border-slate-300 focus:border-violet-600" required />
        </div>
        <div>
          <Label htmlFor="ruolo" className="text-slate-600 text-xs tracking-wide uppercase">Ruolo *</Label>
          <Input id="ruolo" data-testid="form-ruolo" value={form.ruolo} onChange={(e) => set("ruolo", e.target.value)} className="mt-2 h-12 rounded-none border-slate-300 focus:border-violet-600" required />
        </div>
        <div>
          <Label htmlFor="email" className="text-slate-600 text-xs tracking-wide uppercase">Email *</Label>
          <Input id="email" type="email" data-testid="form-email" value={form.email} onChange={(e) => set("email", e.target.value)} className="mt-2 h-12 rounded-none border-slate-300 focus:border-violet-600" required />
        </div>
        <div>
          <Label htmlFor="telefono" className="text-slate-600 text-xs tracking-wide uppercase">Telefono (opzionale)</Label>
          <Input id="telefono" data-testid="form-telefono" value={form.telefono} onChange={(e) => set("telefono", e.target.value)} className="mt-2 h-12 rounded-none border-slate-300 focus:border-violet-600" />
        </div>
        <div>
      ...
      <div className="mt-5">
        ...
        <Textarea id="obiettivo" data-testid="form-obiettivo" rows={3} value={form.obiettivo} onChange={(e) => set("obiettivo", e.target.value)} className="mt-2 rounded-none border-slate-300 focus:border-violet-600" placeholder="Descrivi brevemente obiettivo e contesto..." />
      </div>
      ...
      <div className="flex items-start gap-3 mt-6">
        ...
        <Label htmlFor="privacy" className="text-sm text-slate-500 leading-relaxed cursor-pointer font-normal">
          ...
          <a href="/privacy" className="text-violet-700 underline hover:text-violet-800">Privacy Policy</a>.
        </Label>
      </div>

      <Button
        type="submit" data-testid="form-submit-btn" disabled={loading}
        className="mt-8 bg-violet-700 hover:bg-violet-800 text-white rounded-none px-10 py-3.5 text-sm font-medium tracking-wide uppercase transition-all"
      >
...
export default function ContactSection() {
  return (
    <>
      {/* CTA Band */}
      <section className="py-20 md:py-28 bg-violet-700 text-white">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4">
              Tutto inizia da una conversazione.
            </h2>
            <p className="text-base md:text-lg text-violet-200 max-w-xl mx-auto mb-8">
              Raccontaci il tuo contesto. Ti ricontattiamo con una proposta di primo passo concreto.
            </p>
            <a href="#contatti">
              <Button className="bg-white text-violet-800 hover:bg-white/90 rounded-none px-10 py-3.5 text-sm font-medium tracking-wide uppercase">
            ...
            >
              ...
              <div className="space-y-4 text-sm text-slate-500">
                <div className="flex items-center gap-3">
                  ...
                  <a href="mailto:info@intentionova.ch" className="hover:text-violet-700 transition-colors">info@intentionova.ch</a>
                </div>
                <div className="flex items-center gap-3">
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
            <div className="md:col-span-2">
              <img src={LOGO_URL} alt="Intentio" className="h-10 mb-5 brightness-0 invert opacity-70" />
            ...
            <div>
              ...
              <div className="space-y-2.5 text-sm text-slate-500">
                ...
                <p>6900 Lugano CH</p>
                <a href="mailto:info@intentionova.ch" className="block hover:text-violet-400 transition-colors">info@intentionova.ch</a>
            ...
            <div>
              ...
              <div className="space-y-2.5 text-sm text-slate-500">
                <a href="https://www.intentionova.ch" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 hover:text-violet-400 transition-colors" data-testid="footer-site-link">
                  intentionova.ch <ExternalLink className="h-3 w-3" />
                </a>
                <a href="/privacy" className="block hover:text-violet-400 transition-colors">Privacy Policy</a>
                <a href="/cookie" className="block hover:text-violet-400 transition-colors">Cookie Policy</a>
              </div>
            </div>
          </div>
          <div className="border-t border-white/5 mt-14 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-xs text-slate-600">&copy; {new Date().getFullYear()} Intentio Nova sagl. Tutti i diritti riservati.</p>
